...
import AddressSelection2 from "./AddressSelection2";
...
export default function Cart2() {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      console.log("previewSingleCoupon", response);
      ...
      if (response.giftProduct) {
        await dispatch(
          addItemToCart({
            product_id: response.giftProduct._id,
            quantity: 1,
            price: 0,
          })
        ).unwrap();
        localStorage.setItem("giftProductId", response.giftProduct._id);
  ...
  const handleRemoveCoupon = async () => {
    ...
    try {
      ...
      // Remove gift product if it exists
      if (appliedCoupon?.giftProduct?._id) {
        await dispatch(
          removeItemFromCart(appliedCoupon.giftProduct._id)
        ).unwrap();
  ...
  const mappedCartItems =
    cartItems && Array.isArray(cartItems)
      ? cartItems.map((item, index) => ({
          ...
          isGiftProduct:
            localStorage.getItem("giftProductId") ===
  ...
  const updateQuantity = async (product_id, change) => {
    ...
    try {
      if (newQuantity <= 0) {
        ...
        if (targetItem.product_id === localStorage.getItem("giftProductId")) {
          localStorage.removeItem("giftProductId");
        }
      } else {
        await dispatch(
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row pt-28 lg:pt-42 max-w-7xl px-4 mx-auto mt-7">
      <div className="md:w-3/4 w-full flex flex-col">
        ...
        <div className="flex-grow overflow-y-auto bg-white shadow-inner">
          {!cartInitialized ? (
            ...
            <div className="relative">
              ...
              <AnimatePresence>
                {itemsLoading && (
                  ...
                  >
                    <motion.div
                      className="border-4 border-green-400 border-t-transparent rounded-full w-8 h-8"
                      animate={{ rotate: 360 }}
                      transition={{
                        duration: 1,
                        repeat: Infinity,
      ...
      <div className="md:w-1/4 w-full rounded-2xl">
        {cartInitialized && !isUnauthorized && (
          <motion.div className="bg-gradient-to-r from-green-50 to-green-100 p-4 h-full shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
            {mappedCartItems.length > 0 || appliedCoupon?.giftProduct ? (
              <>
                  ...
                  <div className="space-y-3">
                    ...
                    {!couponsLoading && availableCoupons.length > 0 && (
                      <div className="mb-4 space-y-1">
                        {availableCoupons.filter((c) => c.isApply).length ===
                        1 ? (
                          (() => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <motion.button
                                  ...
                                  whileTap={isApplicable ? { scale: 0.95 } : {}}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    if (isApplicable) {
                          ...
                          <div className="relative" ref={dropdownRef}>
                            <motion.button
                              ...
                              whileTap={{ scale: 0.98 }}
                            >
                              <span>{couponInput || "Available Coupons"}</span>
                              <ChevronDown
                                size={16}
                        ...
                        <div className="flex mt-2">
                          <input
                            ...
                            className="border border-gray-300 rounded-l-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                          />
                          <button
                            className="bg-green-600 hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white px-4 py-2 rounded-r-md font-medium text-sm transition-colors duration-200 flex items-center justify-center"
                          ...
                          >
                            <p className="text-green-600 text-xs font-medium flex items-center justify-between">
                              ...
                              <span className="flex items-center space-x-2">
                                ...
                                {appliedCoupon.giftProduct && (
                                  <span className="font-bold">
                                    Free {appliedCoupon.giftProduct.title}
                                  </span>
                                )}
                                <motion.button
                    ...
                    {couponsLoading && (
                      <motion.div
                        className="flex items-center justify-center gap-2 bg-green-50 text-green-700 rounded-md p-3 my-2 shadow-sm"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ duration: 0.3 }}
                      >
